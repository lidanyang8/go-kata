...
## Kata 15：Go 测试“综合演练”（子测试、并行、Fuzz）

**目标习惯用法：** 表驱动测试、`t.Run`、`t.Parallel`、模糊测试（`go test -fuzz`）  
**难度：** 🟡 中级
...
很多人写测试时会：
- 把类似的用例重复写成一个个“单次测试”；
- 在子测试并行时，不小心错误地捕获了循环变量，导致并行测试不安全；
- 对解析 / 清洗逻辑几乎不做 fuzz 测试。
...
规则：
- 只允许 ASCII 字母、数字和连字符（`-`）；
- 规范化为 HTTP Header 的“标准形式”，例如：`content-type` -> `Content-Type`；
- 对无效输入返回错误。
...
- [ ] 测试**必须**是表驱动的，并使用 `t.Run`。
- [ ] 正确使用**并行子测试**（避免循环变量捕获 bug）。
- [ ] 必须包含一个 **fuzz 测试**，并且保证：
  - 永不 panic；
  - 不会返回包含非法字符的字符串；
  - 对已经是规范形式的字符串，连续调用两次 Normalize 应该是幂等的（结果不变）。

## 🧪 自我校验

- **如果并行子测试偶发性失败（flaky）：** 你很可能错误地捕获了循环变量。
- **如果 fuzz 发现 panic：** 说明你遗漏了某些边界情况。
