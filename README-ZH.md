...
> “我不怕练过一万种踢法一次的人，我只怕把一种踢法练了一万次的人。”
>
> —— 李小龙

## 这个仓库应该是什么？

- Go 语言**容易上手，但难以精通**。从“能跑的代码”到“惯用（idiomatic）的代码”，差别往往体现在一些细节上，比如安全性、内存效率以及并发控制。

- 这个仓库是一组**每日 Kata 练习**：小而独立的编码挑战，用来把特定的 Go 模式练到形成“肌肉记忆”。

## 这个仓库**不**是什么？

- 它不是一门“教你如何编程”的课程，也不是拿 Go 作为学习编程的一般载体，更不是一本“Go 语言入门大全”。

- 它的重点尽量放在：**用 Go 的方式**来挑战常见的软件工程问题，而不是泛泛地讲概念。

- 很多有经验的工程师在其它语言和生产环境中积累了多年的最佳实践。当他们转到 Go 时，经常会面临两个问题：

  - 有没有办法**迁移已有经验**，而不是把多年经验全部清零、从头学起？
  - 如果可以迁移，那**应该重点关注哪些方面**，才能识别与 Go 生态的差异，并按 Go 的期望方式使用它们？

## 如何使用本仓库

1. **选择一个 Kata：** 进入任意 `XX-kata-yy` 目录。
2. **阅读题目：** 打开该目录下的 `README.md`。里面定义了目标、约束条件，以及你必须使用的“惯用模式”。
3. **动手实现：** 在该目录下初始化一个 Go module，然后编写你的解法。
4. **复盘反思：** 将你的方案与提供的“参考实现”（如果有）或列出的核心模式进行对比。
...
### 01）Context、取消机制与快速失败并发

围绕真实场景的并发模式：防止泄漏、实现背压，以及在取消时快速失败。

- [01 - 快速失败的数据聚合器](./01-context-cancellation-concurrency/01-concurrent-aggregator)
- [03 - 优雅关停的 HTTP 服务器](./01-context-cancellation-concurrency/03-graceful-shutdown-server)
- [05 - 感知 Context 的错误传播器](./01-context-cancellation-concurrency/05-context-aware-error-propagator)
- [07 - 受限速控制的扇出客户端](./01-context-cancellation-concurrency/07-rate-limited-fanout)
- [09 - 防止缓存击穿的单飞 TTL 缓存](./01-context-cancellation-concurrency/09-single-flight-ttl-cache)
- [10 - 具备背压与 errors.Join 的工作池](./01-context-cancellation-concurrency/10-worker-pool-errors-join)
- [14 - 无泄漏的调度器](./01-context-cancellation-concurrency/14-leak-free-scheduler)
- [17 - 感知 Context 的通道发送器（无生产者泄漏）](./01-context-cancellation-concurrency/17-context-aware-channel-sender)

---

### 02）性能、内存分配与吞吐

聚焦内存效率、分配控制以及高吞吐数据路径的练习。

- [02 - 分片加锁的并发 Map](./02-performance-allocation/02-concurrent-map-with-sharded-locks)
- [04 - 零分配 JSON 解析器](./02-performance-allocation/04-zero-allocation-json-parser)
- [11 - 支持超长行的 NDJSON 流读取器](./02-performance-allocation/11-ndjson-stream-reader)
- [12 - 使用 sync.Pool 的缓冲区中间件](./02-performance-allocation/12-sync-pool-buffer-middleware)
...
围绕 HTTP 客户端 / 服务端模式、中间件组合以及生产级代码卫生的练习。

- [06 - 基于接口的中间件链](./03-http-middleware/06-interface-based-middleware-chain)
- [16 - HTTP 客户端卫生包装器](./03-http-middleware/16-http-client-hygiene)

---

### 04）错误：语义、包装与边界情况

现代 Go 错误处理：重试、清理、包装以及那些著名的“坑”。

- [08 - 尊重 Context 的重试策略](./04-errors-semantics/08-retry-backoff-policy)
- [19 - 清理链（defer + LIFO + 错误保留）](./04-errors-semantics/19-defer-cleanup-chain)
- [20 - “nil != nil” 接口陷阱（带类型的 nil 错误）](./04-errors-semantics/20-nil-interface-gotcha)
...
围绕可移植二进制、可测试的文件系统代码与开发 / 生产环境一致性的练习。

- [13 - 与文件系统无关的配置加载器（io/fs）](./05-filesystems-packaging/13-iofs-config-loader)
- [18 - 使用 embed.FS 的 Dev/Prod 切换](./05-filesystems-packaging/18-embedfs-dev-prod-switch)

---

### 06）测试与质量保障

地道的 Go 测试：表驱动测试、并行与 fuzzing。

- [15 - Go 测试框架（子测试、并行、模糊测试）](./06-testing-quality/15-testing-parallel-fuzz-harness)
